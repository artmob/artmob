...
/* 					
						// SHYAM: COMMENTING THIS OUT BLOCK PREVENTED CHANGING THE INFO VALUES, BUT THE WORK-2 TITLE IS COMING IN INCORRECTLY 
...
function createTier0Menu(ctarget,type)
{
      ...
      var output = '<div id="cvalues-1-'+divcounter+'" class="cvalues"><select name="'+rolename+'" class="select-'+classname+'"><option value="">Select '+textlabel+'</option>';
      
      if (type == "roles"){
      for (var contrib in contributor_roles) {
	  	output += '<option value="'+contrib+'">'+contributor_roles[contrib].slice(0,20)+'</option>';
	  }
	  output += "</select>";
      output += '<select name="cname-1-1" class="select-cname">';
	  ...
	  jQ14.ajax({
		  ...
		  success: function(data) {
			  ...
			  output += "<option value=''><em>Select Name</em></option>";
			  ...
			  for (var key in contributors) {
				if (key != undefined) {
				  ...
				  /*if (cookie != "")
				  	alert(cookie);*/
				  output += "<option value='" + key + "'>" + contributors[key] + "</option>";
				}
			  }		  
		  }
	  });
	  output += '</select><span class="add-new-contributor-container" style="display:none"><input type="text" class="fname-cname" value="First Name"> <input type="text" value="Last Name" class="lname-cname"><input type="checkbox" class="cc-cname" value="1" />&nbsp;Corporate Contributor</span><input type="button" value="Or add new name" class="add-new-contributor" /><span class="delete-contributor"><input type="button" class="delete-contributor-btn" value="Delete"/></span></div>';	 
	  }
	  else {
	  	for (var field in fields) {
	  		if (field != undefined) {
	  			output += "<option value='" + field + "'>" + fields[field]["label"] + "</option>";
	  		}
	  	}
     	output += '</select><span class="infoval-container"><input type="text" value="Enter Information" name="info-1-1" class="infoval infoval-text"></span><span class="delete-info"><input type="button" class="delete-info-btn" value="Delete"/></span></div>';	 
	  	
	  }
	  
	  var prevhtml = jQ14(ctarget).html();
	  //output += prevhtml + output;
	  jQ14(ctarget).html(output);
}

function menuCreateInitialWorkTypes(ft) {
  //SPECIAL EXCEPTION FOR WORK1 - HAVE TO DO OTHER CHECKS ON CHANGE TO PRESERVE DATA
  parent_ft = ft;
  jQ14("#work-1 .menu .worksmenu select").unbind('change');
  jQ14("#work-1 .menu .worksmenu select").bind('change',function() { 
   // alert(jQ14('#cvalues-0-1 > .select-crole option:selected').val());  
	...
	/*if (add_works != "")
	  alert(add_works);*/
	menuBuildCollection(branch.getBranch(value),'work-1 > .innerwork > .nested-works','nest');	
    if (add_works != ""){
...
function checkNestedValues() {
    ...
    if (r['works'][i] != null && typeof r['works'][i][0] != "undefined") {       
	  ...
	  jQ14('.work > .identifiers .id').each(function() {
	    ...
	    if (jQ14(nestedid + " div.work:nth-child(" + sequence + ")").attr('id') == undefined)
	    	target = "#work-2";		
	  var wt = jQ14("#work-"+parent+" .menu .worksmenu select option:selected").val();
	  ...
	  if (jQ14(childtarget + " .select-crole").val() == "")
         		jQ14(childtarget + " .delete-contributor .delete-contributor-btn").trigger('click');
         }       
      } 
  ...
  if (jQ14(this).val().indexOf("invalid") == -1) {
                                 jQ14(this).children("option").each(function(){
                                  ...
                                  jQ14("#errortext").hide();
 								  //jQ14('.select-crole').trigger('change');
								  //jQ14('.select-cfield').trigger('change');
      ...
      jQ14("#work-collection-init .work").each(function() {
        ...
        info.push([{
          ...
          "type" : 'textfield'
            ...
            if (cvalues.length > 0 && sequence == 1) {
              cvalues.each(function() {
                 ...
                 if (jQ14(cid +' > .select-crole option:selected').val() != null) {
                   
                   contributors.push([{
                       "role" : jQ14(cid +' > .select-crole option:selected').val(),
                       "existing" : jQ14(cid +' > .select-cname option:selected').val(),
      ...
      {report: JSON.stringify(report) },
       function (data) {
         var result = Drupal.parseJson(data);
          jQ14('#info-summary').append("<div class='message'>Your information has been successfully saved.</div>");
          if (debug == true) {
            jQ14('#info-summary').append("report: " + JSON.stringify(report));//!!!comment out when live
          }
         return false;
    });
}
